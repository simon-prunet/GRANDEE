...
'''
# --------------------------------------------
# Code entirely borrowed from Kai Zhang, 
# converted to 1D
# --------------------------------------------
'''
...
def pixel_unshuffle(input, upscale_factor):
    r"""Rearranges elements in a Tensor of shape :math:`(*, C, rH)` to a
    tensor of shape :math:`(*, rC, H)`.
    ...
    unshuffle_out = input_view.permute(0, 3, 1, 2).contiguous() 
    return unshuffle_out.view(batch_size, channels, out_height)

# --------------------------------------------
# implement pixel_shuffle in 1d
# --------------------------------------------

def pixel_shuffle(input, downscale_factor):
    r"""Rearranges elements in a Tensor of shape :math:`(*, rC, H)` to a 
    tensor of shape :math:`(*, C, rH)`
    """
    batch_size, channels, in_height = input.size()
    channels //= downscale_factor
    input_view = input.contiguous().view(
        batch_size, downscale_factor, channels, in_height)
    out_height = in_height * 2
    shuffle_out = input_view.permute(0,2,3,1).contiguous() ## CHECK PERMUTATION
...
# --------------------------------------------
# I (SP) don't understand everything here.. Just changed BatchNorm2d to BatchNorm1d
# If anything needs to be changed in forward() it has not been done.
...
def upsample_pixelshuffle(in_channels=64, out_channels=3, kernel_size=3, stride=1, padding=1, bias=True, mode='2R', negative_slope=0.2):
    ...
    up1 = conv(in_channels, out_channels * int(mode[0]), kernel_size, stride, padding, bias, mode='C'+mode, negative_slope=negative_slope)
    return up1
